...
def create_admin_product():
    ...
    try:
        ...
        # Get the created_by from the admin decorator
        from flask import g
        admin_user = g.get('current_admin_user', {})
        created_by = admin_user.get('user_id', ObjectId())
        
        # Build the arguments for Product.create()
        create_args = {
            'name': data.get('name'),
            'description': data.get('description', ''),
            'price': str(data.get('price')),  # Convert to string as Product model expects string
        ...
        if data.get('images') and isinstance(data['images'], list):
            create_args['images'] = data['images']
        elif data.get('image'):
            # Convert single image to array
            create_args['images'] = [data['image']]
    ...
    try:
        ...
        for admin_field, model_field in field_mappings.items():
            if admin_field in data:
                # Special handling for price - convert to string
                if model_field == 'price':
                    update_data[model_field] = str(data[admin_field])
                else:
